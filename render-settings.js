...
// The settings screens that own a setting: the two-level hub, its search, every
// renderSettings* page with a write handler behind it, the earth-resistance
// calculator, and the photo export/import section of the Backup page.
//
// v73: the read-only help screens moved OUT to render-help.js — About (and its
// changelog), Glossary, Contact, the bug-report sheet markup and the cloud-prep
// stub pages. Those pages still call renderSettingsSubHeader() from this file.
// If you are looking for the About changelog, it is in render-help.js now.
  ...
  return `
    <div class="screen">
      ...
      <div class="settings-section">
        ...
        ${paired ? `<p class="muted">While this is on, the app keeps the on-screen keyboard away from the asset box. If you need to type a number by hand and your scanner isn't with you, turn this switch back off. See "Typing while a scanner is connected" below.</p>` : ''}
      </div>
      ...
      <div class="settings-section">
        ...
        <p class="muted">If the scanner is switched off, flat or out of range and you just want to type, turn the <strong>Scanner paired</strong> switch above off. That's what's holding the keyboard back, and turning it off releases the asset box straight away. Turn it on again when the scanner is back.</p>
      </div>
...
function renderSettingsBackup() {
  ...
  // Export, because "put it back" and "save a copy first" are the same thought.
  const undoCount = (typeof apostropheRepairUndoCount === 'function') ? apostropheRepairUndoCount() : 0;
  const undoRepairBlock = undoCount > 0 ? `
      <div class="settings-section">
        <h2 class="h2">Apostrophe correction</h2>
        <p class="muted">This update corrected ${undoCount} saved location${undoCount === 1 ? '' : 's'} and item type${undoCount === 1 ? '' : 's'} where an apostrophe had put a stray capital in the middle of a word &mdash; for example "Bob'S Office" is now "Bob's Office". If any of them looked better the way they were, you can put them all back.</p>
